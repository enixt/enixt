...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeEnixtAmount"))
        settings.setValue("nAnonymizeEnixtAmount", 1000);
    nAnonymizeEnixtAmount = settings.value("nAnonymizeEnixtAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeEnixtAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeEnixtAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeEnixtAmount:
            return QVariant(nAnonymizeEnixtAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeEnixtAmount:
            nAnonymizeEnixtAmount = value.toInt();
            settings.setValue("nAnonymizeEnixtAmount", nAnonymizeEnixtAmount);
            emit AnonymizeEnixtAmountChanged(nAnonymizeEnixtAmount);
            break;
        default:
            break;
        }
