...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: enixt.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: enixtd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeenixtamount=<n> " + _("Keep N Enixt anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Enixt is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Enixt is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Enixt version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Enixt server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Enixt") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Enixt to complete") << "\n";
    ...
    nAnonymizeEnixtAmount = GetArg("-anonymizeenixtamount", 0);
    if(nAnonymizeEnixtAmount > 999999) nAnonymizeEnixtAmount = 999999;
    if(nAnonymizeEnixtAmount < 2) nAnonymizeEnixtAmount = 2;
    ...
    LogPrintf("Anonymize Enixt Amount %d\n", nAnonymizeEnixtAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ENIX+1000 == (.1ENIX+100)*10
       10ENIX+10000 == (1ENIX+1000)*10
    */
